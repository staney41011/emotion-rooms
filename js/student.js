...
function fillEmotion(word) {
  ...
  if (!target) {
    updateSuggestionStatus('最多可以填 6 個感受，請先修改或刪除其中一個。');
    return;
  }
...
function renderRoomLabel() {
  ...
  const label = getRoomLabel(currentRoom.id);
  if (roomExperienceTitle) roomExperienceTitle.textContent = label;
  document.title = `${label}｜情緒包廂`;
}
...
window.addEventListener('emotion-room-labels-changed', renderRoomLabel);
